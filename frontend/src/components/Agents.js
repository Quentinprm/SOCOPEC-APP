...
export default class Agents extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`${Urls.api}/user`).then(response => {
      ...
      return (
        <div className="car">
          <table className="carInfos">
            ...
            <tr>
              <td>Mail:{car.Email}</td>
              <td>Password: ********</td>
              <td>Tel.: {car.PhoneNumber}cm</td>
            </tr>
            <tr>
              <td>Fax: {car.Fax}</td>
              <td>Mobile: {car.Mobile}ch</td>
              <td></td>
            </tr>
          </table>
    ...
    return (
      <div>
        ...
        >
          <Modal.Dialog>
            <Modal.Header>
              <Modal.Title>Ajouter un agent</Modal.Title>
            </Modal.Header>
            <input
              type="text"
              placeholder="nom"
        ...
        <div id="carslist" style={{ opacity: !this.state.show ? '1' : '0.05' }}>
          ...
          <div>
            {this.state.cars.length} agents trouvés{' '}
            <Button id="searchButton" onClick={this.search}>
              Rechercher
            </Button>
            <FormGroup controlId="formControlsSelect" id="sortBar">
              <FormControl componentClass="select" placeholder="select">
                <option value="modele">Nom</option>
                <option value="agence">Prénom</option>
                <option value="annee">Mail</option>
                <option value="poids">Agence</option>
              </FormControl>
            </FormGroup>
          </div>
