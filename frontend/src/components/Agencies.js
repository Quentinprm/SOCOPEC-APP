...
export default class Agencies extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`${Urls.api}/agency`).then(response => {
      ...
      return (
        <div className="car">
          ...
          <table className="carInfos">
            ...
            <tr>
              <td colspan="2">Adresse:{car.Adress}</td>
            </tr>
            <tr>
              <td>Ville: {car.City}</td>
              <td>Tel: {car.PhoneNumber}</td>
              <td>Fax: {car.Fax}</td>
            </tr>
          </table>
